{{-- Accreditation / partner logo wall.
     DEMO wordmark badges — replace SVGs in /public/images/accreditations/ with official brand assets (same filenames). --}}
@php
    $accreditors = [
        ...
        ['file' => 'oxford-aqa',        'name' => 'Oxford International AQA',      'tag' => 'Registered Centre'],
...
<div class="mt-10 grid grid-cols-2 md:grid-cols-3 {{ !empty($compact) ? 'lg:grid-cols-6' : '' }} gap-4 sm:gap-6">
    ...
    <div class="reveal bg-white rounded-sm border border-beige-200 shadow-sm hover:shadow-md transition-shadow p-5 flex flex-col items-center text-center">
        <img src="/images/accreditations/{{ $a['file'] }}.svg" alt="{{ $a['name'] }} logo" class="h-14 w-auto" width="220" height="80" loading="lazy">
        @if (empty($compact))
